...
// The return value of this function is referenced to an arbitrary epoch,
// and as such is only suitable for comparison with other unpacked time values
...
void MoonlightInstance::ProfilerPrintPackedDelta(const char* message, uint32_t packedTimeA, uint32_t packedTimeB) {
    printDeltaAboveThreshold(message, (uint32_t)(ProfilerUnpackTime(packedTimeB) - ProfilerUnpackTime(packedTimeA)));
}
...
void MoonlightInstance::ProfilerPrintDelta(const char* message, uint64_t timeA, uint64_t timeB) {
